...
export function createBaseFileSystem(): FileSystem {
  ...
  const osdcUser = fileSystem.addDirectory(users, 'osdc', false, true);

  const desktop = fileSystem.addDirectory(osdcUser, 'Desktop', false, true);
  const documents = fileSystem.addDirectory(osdcUser, 'Documents', false, true, documentsFolderIcon);
  const trashCanIcon = { src: '/icons/trash-icon.png', alt: 'Trash can icon' };
  const trash = fileSystem.addDirectory(osdcUser, 'Trash', false, true, trashCanIcon);
...
function entriesWithinSelection(entries: Point[], x: number, y: number, dimensions: { width: number, height: number }): number {
  ...
  // The width & height are used for both the new incoming entry as the already existing entries.
