import React, { useEffect, useRef, useState } from 'react';
import { ApplicationConfig, ApplicationManager } from '@/applications/ApplicationManager';
import styles from './Dock.module.css';
import { aboutConfig } from '@/applications/About/About';
import { finderConfig } from '@/applications/Finder/Finder';
import { debugConfig } from '@/applications/Debug/DebugApplication';
import { Window, WindowCompositor } from '../WindowManagement/WindowCompositor';
import { WindowEvent } from '../WindowManagement/WindowEvents';
import { ApplicationIcon, FileSystem } from '@/apis/FileSystem/FileSystem';
import { SystemAPIs } from '../OperatingSystem';
import { FileSystemItemDragDrop, FileSystemItemDragEnter, FileSystemItemDragEvent, FileSystemItemDragLeave } from '@/apis/DragAndDrop/DragAndDrop';
import { contactConfig } from '@/applications/Contract/Contact';
import { skillsConfig } from '@/applications/Skills/Skills';
import { algorithmVisualizerConfig } from '@/applications/AlgorithmVisualizer/AlgorithmVisualizer';
import { terminalConfig } from '@/applications/Terminal/TerminalApplication';
import { notesConfig } from '@/applications/Notes/Notes';
import { doomConfig } from '@/applications/Doom/Doom';

const DockApplications = [
  finderConfig,
  aboutConfig,
  contactConfig,
  algorithmVisualizerConfig,
  skillsConfig,
  terminalConfig
];

const DebugDockApplications = [
  ...DockApplications,
  debugConfig
];

const ActiveIcon = 'data:image/png;base64,';

type ApplicationDockItem = { kind: 'application', config: ApplicationConfig, active: boolean, onClick: () => void }
type MinimizedApplicationDockItem = { kind: 'minimized_application', title: string, config: ApplicationConfig, onClick: () => void }
type SeparatorDockItem = { kind: 'separator' }
type DirectoryDockItem = { kind: 'directory', title: string, path: string, icon: ApplicationIcon, onClick: () => void}

type DockItem = ApplicationDockItem | MinimizedApplicationDockItem | SeparatorDockItem | DirectoryDockItem;

function DockItemViewApplication(props: { item: ApplicationDockItem }) {
  const { item } = props;

  return (<>
    <button className={styles['dock-application']} onClick={() => item.onClick()} data-tooltip={item.config.displayName}>
      <img
        className={styles['dock-app-image']}
        src={item.config.appIcon.src}
        alt={item.config.appIcon.alt}
        width={64}
        height={64}
        draggable={false}
      />
      <div className={[styles.status, item.active ? styles.active : styles.inactive].join(' ')}>
        {item.active ? <img draggable={false} className={styles['dock-app-active-image']} src={ActiveIcon} /> : <></>}
      </div>
    </button>
  </>)
}

function DockItemMinimizedApplication(props: { item: MinimizedApplicationDockItem }) {
  const { item } = props;

  return (<>
    <button className={styles['dock-application']} onClick={() => item.onClick()} data-tooltip={item.config.displayName}>
      <img
        className={styles['dock-app-image']}
        src={item.config.appIcon.src}
        alt={item.config.appIcon.alt}
        width={64}
        height={64}
        draggable={false}
      />
      <div className={[styles.status].join(' ')}></div>
    </button>
  </>)
}

function DockItemSeparator() {
  return (<div className={styles.separator}></div>)
}

function DockItemDirectory(props: { item: DirectoryDockItem, fileSystem: FileSystem }) {
  const { item, fileSystem } = props;
  const ref = useRef<HTMLButtonElement>(null);
  const [hovered, setHovered] = useState<boolean>(false);

  function onFileEnter(): void {
    setHovered(true);
  }

  function onFileLeave(): void {
    setHovered(false);
  }

  function onFileDrop(evt: FileSystemItemDragEvent): void {
    setHovered(false);

    const nodes = evt.detail.nodes;

    const parent = fileSystem.getDirectory(item.path);
    if (!parent.ok) { return; }

    nodes.forEach(node => { fileSystem.moveNode(node.item, parent.value); });
  }

  useEffect(() => {
    if (!ref.current) { return; }
    const container = ref.current;

    container.addEventListener(FileSystemItemDragEnter, onFileEnter as EventListener);
    container.addEventListener(FileSystemItemDragLeave, onFileLeave as EventListener);
    container.addEventListener(FileSystemItemDragDrop, onFileDrop as EventListener);

    return () => {
      container.removeEventListener(FileSystemItemDragEnter, onFileEnter as EventListener);
      container.removeEventListener(FileSystemItemDragLeave, onFileLeave as EventListener);
      container.removeEventListener(FileSystemItemDragDrop, onFileDrop as EventListener);
    }
  }, []);

  const buttonStyle = [styles['dock-application'], hovered ? styles['hovered'] : ''].join(' ');

  return (<>
    <button
      ref={ref}
      data-drop-point="true"
      className={buttonStyle}
      onClick={() => item.onClick()}
      data-tooltip={item.title}>
      <img
        className={styles['dock-app-image']}
        src={item.icon.src}
        alt={item.icon.alt}
        width={64}
        height={64}
        draggable={false}
      />
      <div className={styles.status}></div>
    </button>
  </>)
}

function DockItemView(props: { item: DockItem, fileSystem: FileSystem }) {
  const { item, fileSystem } = props;

  switch (item.kind) {
    case 'application': return <DockItemViewApplication item={item} />;
    case 'minimized_application': return <DockItemMinimizedApplication item={item} />;
    case 'separator': return <DockItemSeparator />;
    case 'directory': return <DockItemDirectory item={item} fileSystem={fileSystem} />;
  }
}

function formatMonitorClock(date: Date): string {
  return date.toLocaleTimeString([], {
    hour: '2-digit',
    minute: '2-digit',
  });
}

export function Dock(props: {
  apis: SystemAPIs,
  manager: ApplicationManager,
  windowCompositor: WindowCompositor,
  monitorMode?: boolean,
}) {
  const { apis, manager, windowCompositor, monitorMode = false } = props;
  const systemService = apis.system;
  const fileSystem = apis.fileSystem;

  const [dockItems, setDockItems] = useState<DockItem[]>(constructDock(manager, windowCompositor, systemService.isDebug()));
  const [directoryItems] = useState<DirectoryDockItem[]>(constructDirectoryItems());
  const [taskWindows, setTaskWindows] = useState<Window[]>(windowCompositor.listWindows());
  const [time, setTime] = useState(new Date());

  function constructDock(manager: ApplicationManager, windowCompositor: WindowCompositor, debug: boolean): DockItem[] {
    let content: DockItem[] = [];

    const dockApplications    = !debug ? DockApplications : DebugDockApplications;

    const activeApplications  = manager.listApplications().map(x => x.config());
    const minimizedWindows    = windowCompositor.listMinimizedWindows();

    {
      const items: Record<string, { config: ApplicationConfig, active: boolean }> = {};

      dockApplications.forEach(x => items[x.appName] = {
        config: x,
        active: false
      });

      activeApplications.forEach(x => items[x.appName] = {
        config: x,
        active: true
      });

      Object.values(items)
        .sort((a, b) => { 
          // Bubble sorting time 😎
          const aPriority = a.config.dockPriority ?? 0;
          const bPriority = b.config.dockPriority ?? 0;

          return aPriority - bPriority;
        }).forEach(item => {
          content.push({
            kind: 'application',
            config: item.config,
            active: item.active,
            onClick: () => { manager.open(item.config.path + item.config.appName); }
          });
        });
    }

    content.push({ kind: 'separator' });

    {
      function onClickMinimizedApplicationWindow(applicationWindow: Window) {
        applicationWindow.minimized = false;
        windowCompositor.update(applicationWindow);
        windowCompositor.focus(applicationWindow.id, true);
      }

      minimizedWindows
        .slice(0, Math.min(minimizedWindows.length, 5))
        .forEach(window => {
        content.push({
          kind: 'minimized_application',
          title: window.title,
          config: window.application.config(),
          onClick: () => onClickMinimizedApplicationWindow(window)
        });
      });
    }

    return content;
  }

  function constructDirectoryItems(): DirectoryDockItem[] {
    let content: DirectoryDockItem[] = [];

    function onClickDirectory(path: string) {
      manager.open(`/Applications/Finder.app ${path}`);
    }

    content.push({
      kind: 'directory',
      title: 'Applications',
      path: '/Applications',
      icon:  { src: '/icons/icon-applications-folder.png', alt: 'File icon' },
      onClick: () => onClickDirectory('/Applications'),
    });
    
    content.push({
      kind: 'directory',
      title: 'Documents',
      path: '/Users/osdc/Documents',
      icon:  { src: '/icons/icon-documents-folder.png', alt: 'File icon' },
      onClick: () => onClickDirectory('/Users/osdc/Documents'),
    });

    content.push({
      kind: 'directory',
      title: 'Bin',
      path: '/Users/osdc/Trash',
      icon:  { src: '/icons/trash-icon.png', alt: 'Trash can' },
      onClick: () => onClickDirectory('/Users/osdc/Trash'),
    });
  
    return content;
  }

  function handleApplicationManager() {
    setDockItems(constructDock(manager, windowCompositor, systemService.isDebug()));
    setTaskWindows(windowCompositor.listWindows());
  }

  function handleWindowCompositor(windowEvent: WindowEvent) {
    setDockItems(constructDock(manager, windowCompositor, systemService.isDebug()));
    setTaskWindows(windowCompositor.listWindows());
  }

  useEffect(() => {
    const applicationManagerUnsubscribe = manager.subscribe(handleApplicationManager);
    const windowCompositorUnsubscribe = windowCompositor.subscribe(handleWindowCompositor);

    return () => {
      applicationManagerUnsubscribe();
      windowCompositorUnsubscribe();
    }
  }, []);

  useEffect(() => {
    if (!monitorMode) { return; }

    const interval = window.setInterval(() => setTime(new Date()), 1000);

    return () => {
      window.clearInterval(interval);
    };
  }, [monitorMode]);

  if (monitorMode) {
    function openApplication(config: ApplicationConfig) {
      manager.open(config.path + config.appName);
    }

    function onClickTaskWindow(applicationWindow: Window) {
      if (applicationWindow.minimized) {
        applicationWindow.minimized = false;
        windowCompositor.update(applicationWindow);
        windowCompositor.focus(applicationWindow.id, true);
        return;
      }

      if (applicationWindow.focused) {
        windowCompositor.minimize(applicationWindow.id);
        return;
      }

      windowCompositor.focus(applicationWindow.id, true);
    }

    const quickLaunch = [
      finderConfig,
      notesConfig,
      terminalConfig,
      doomConfig,
      skillsConfig,
      algorithmVisualizerConfig,
    ];

    return <>
      <div className={styles.monitorShell}>
        <div className={styles.monitorTaskbar}>
          <div className={styles.monitorLaunchGroup}>
            <div className={styles.monitorStripLabel}>TOOLS</div>
            <div className={styles.monitorQuickLaunch}>
              {quickLaunch.map((application) => (
                <button
                  key={application.appName}
                  className={styles.monitorQuickLaunchButton}
                  onClick={() => openApplication(application)}
                  title={application.displayName}
                >
                  <img
                    className={styles.monitorQuickLaunchIcon}
                    src={application.appIcon.src}
                    alt={application.appIcon.alt}
                    width={16}
                    height={16}
                    draggable={false}
                  />
                </button>
              ))}
            </div>
          </div>

          <div className={styles.monitorTaskList}>
            {taskWindows.map((applicationWindow) => {
              const config = applicationWindow.application.config();
              const classes = [
                styles.monitorTaskButton,
                applicationWindow.focused && !applicationWindow.minimized ? styles.monitorTaskButtonActive : "",
                applicationWindow.minimized ? styles.monitorTaskButtonMinimized : "",
              ].join(" ").trim();

              return (
                <button
                  key={applicationWindow.id}
                  className={classes}
                  onClick={() => onClickTaskWindow(applicationWindow)}
                  title={applicationWindow.title}
                >
                  <img
                    className={styles.monitorTaskIcon}
                    src={config.appIcon.src}
                    alt={config.appIcon.alt}
                    width={16}
                    height={16}
                    draggable={false}
                  />
                  <span className={styles.monitorTaskLabel}>{applicationWindow.title}</span>
                </button>
              );
            })}
          </div>

          <div className={styles.monitorClock}>{formatMonitorClock(time)}</div>
        </div>
      </div>
    </>;
  }

  return <>
    <div className={styles.dock}>
      <div data-drop-point="true" className={styles.dockContainer}>
        {dockItems.map((item, i) => <DockItemView key={i} item={item} fileSystem={fileSystem} />)}
        {directoryItems.map((item, i) => <DockItemDirectory key={i} item={item} fileSystem={fileSystem} />)}
      </div>
    </div>
  </>
}
