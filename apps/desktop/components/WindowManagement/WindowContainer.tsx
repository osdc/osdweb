...
const DoubleClickHeaderForMaximizingTimeInMs = 350;
    ...
    for (const axis of axes) {
      switch (axis) {
        case 'n': {
          ...
          const maxHeight = windowHeight + windowY;

          const clampedY = clamp(windowY - deltaY, 0, maxDelta);
          const clampedHeight = clamp(windowHeight + deltaY, windowMinHeight, maxHeight);

          windowData.y = clampedY;
          windowData.height = clampedHeight;
        } break;
        case 'e': {
          const maxWidth = parent.offsetWidth - windowX;
          const clampedWidth = clamp(windowWidth - deltaX, windowMinWidth, maxWidth);
          windowData.width = clampedWidth;
        } break;
        case 's': {
          const maxHeight = parent.offsetHeight - windowY;
          const clampedHeight = clamp(windowHeight - deltaY, windowMinHeight, maxHeight);
          windowData.height = clampedHeight;
        } break;
        case 'w': {
          const maxDelta = windowX + (windowWidth - windowMinWidth);
          const maxWidth = windowWidth + windowX;

          const clampedX = clamp(windowX - deltaX, 0, maxDelta);
          const clampedWidth = clamp(windowWidth + deltaX, windowMinWidth, maxWidth);
...
const WindowHeader = (props: {
  ...
  function onPointerMove(evt: PointerEvent) {
    ...
    const deltaX = evt.clientX - cursorRef.x;
    const deltaY = evt.clientY - cursorRef.y;

    const maxWindowX = Math.max(0, parent.offsetWidth - windowData.width);
    const maxWindowY = Math.max(0, parent.offsetHeight - windowData.height);

    windowData.x = clamp(windowRef.x + deltaX, 0, maxWindowX);
    windowData.y = clamp(windowRef.y + deltaY, 0, maxWindowY);
...
export default function WindowContainer(props: { window: Window, WindowApp: WindowApplication, windowCompositor: WindowCompositor, parent: HTMLDivElement | null }) {
  ...
  useEffect(() => {
    const resizeObserver = new ResizeObserver(() => {
      if (!maximized.current) { return; }

      const liveWindow = windowCompositor.getById(window.id);
      if (!liveWindow) { return; }

      liveWindow.x = 0;
      liveWindow.y = 0;
      liveWindow.width = parent.offsetWidth;
      liveWindow.height = parent.offsetHeight;

      windowCompositor.update(liveWindow, { moved: true, resized: true });
    });
  ...
  }, [parent, window.id, windowCompositor]);
