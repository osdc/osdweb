...
const DesktopToolTargets = {
  workspace: '/Applications/Finder.app /Users/osdc/',
  finder: '/Applications/Finder.app /Users/osdc/',
...
function openDesktopTool(tool: string | null | undefined): void {
  ...
  const target = DesktopToolTargets[tool as keyof typeof DesktopToolTargets];
  if (!target) { return; }

  applicationManager.open(target);
}
...
function handleParentResponsesClosure(
  ...
  return function(event: MessageEvent) {
    if (event.data && event.data.method === 'osdc_launch_tool') {
      openDesktopTool(event.data.tool);
      return;
    }
...
export const OperatingSystem = () => {
  ...
  useEffect(() => {
    ...
    const monitorWallpaperUrl = searchParams.get('wallpaper');
