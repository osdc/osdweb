...
export class TerminalManager implements TerminalConnector {
  ...
  constructor(private terminal: Terminal, private domElement: HTMLElement, applicationManager: BaseApplicationManager, apis: SystemAPIs) {
    ...
    const cols = Math.max(20, Math.floor(domElement.clientWidth / cellWidth));
    const rows = Math.max(6, Math.floor(domElement.clientHeight / cellHeight));
  ...
  private onResize(): void {
    ...
    const cell: { height: number, width: number } | undefined = core?._renderService?.dimensions?.css?.cell;

    if (!cell || !Number.isFinite(cell.width) || !Number.isFinite(cell.height) || cell.width <= 0 || cell.height <= 0) {
      return;
    }

    const cols = Math.max(20, Math.floor(terminalContainer.clientWidth / cell.width));
    const rows = Math.max(6, Math.floor(terminalContainer.clientHeight / cell.height));
